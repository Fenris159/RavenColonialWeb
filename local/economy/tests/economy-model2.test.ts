import { applyParentHubSubStrongLink } from "../../../src/economy/economy-documented";
import { applyBuffs, calculateAgricultureStrongLinkContribution, calculateColonyEconomies2, getColonyEconomyBeforeWeakLinks, isGroundOrbitColonyPair } from "../../../src/economy";
import { applyAgricultureSettlementFloor } from "../../../src/economy/economy-ag-heuristics";
...
describe("applyBuffs agriculture", () => {
  ...
  it("does not apply icy agriculture penalties on the port's own agriculture row", () => {
    const map = createEconomyMap();
  ...
  it("does not apply icy or tidal agriculture penalties on the port's own agriculture row", () => {
    const map = createEconomyMap();
  ...
  it("keeps tidally locked WW agriculture above the body-type floor without tidal body buffs", () => {
    const map = createEconomyMap();
  ...
  it("does not apply decrease modifiers during intrinsic body buffs", () => {
    const map = { ...createEconomyMap(), agriculture: 0.4 };
    const site = createTidallyLockedSite();

    applyBuffs(map, site, false);

    expect(map.agriculture).toBe(0.4);
    expect(site.economyAudit).toEqual([]);
  });

  it("floors medium and small tidal agriculture settlements at 60%", () => {
    const map = { ...createEconomyMap(), agriculture: 0.6 };
  ...
  it("keeps large agriculture settlements at intrinsic strength without tidal body buffs", () => {
    const map = createEconomyMap();
    const site = createTidallyLockedSite(BT.hmc);
...
describe("calculateAgricultureStrongLinkContribution", () => {
  ...
  it("does not stack the icy Atropos preset economies on BIO body economies", () => {
    ...
    expect(site.economies!.agriculture).toBe(1.65);
...
describe("agriculture weak link caps and floors", () => {
  ...
  it("parent hub sub-strong uses parent hub tier, not subordinate port tier", () => {
    const sys = { bodies: [], reserveLevel: "pristine" } as unknown as SysMap2;
    const parent = {
      id: "hub",
      name: "Refinery Hub",
      sys,
      type: { inf: "refinery" as Economy, tier: 2, buildClass: "hub" },
    } as unknown as SiteMap2;
    const site = {
      id: "port",
      name: "Sub Port",
      ...
      economyAudit: [],
      sys,
      type: { inf: "colony" as Economy, tier: 1, fixed: undefined, buildClass: "starport" },
    } as unknown as SiteMap2;
    const map = { ...createEconomyMap(), refinery: 1 };

    applyParentHubSubStrongLink(map, site, ["hub", "port"]);

    expect(map.refinery).toBe(2.2);
    expect(site.economyAudit).toContainEqual(
      expect.objectContaining({
        delta: 0.8,
        inf: "refinery",
