...
| Weak-link application | `applyWeakLinks` — sorted until agriculture budget exhausted; skips sources already in `strongSites` (incl. hub grandchildren); colony sources use `primaryEconomy` only |
| Weak-link contributors | Tiered ports/hubs when subordinate; **colony body primaries** weak-link **agriculture only** outward; **star-body primary ports** do not emit other weak links (Fort Snailing); relay (`enodia` / `ichnaea`) and **security installations** weak-link system-wide; **demeter** space farms weak-link cross-body when not anchored to a same-body colony port (strong stays local/cluster); military hub installations (`alastor` / `vacuna`) never weak-link outward |
| Relay economy apply | Relay weak **High Tech** +5% on **outposts** and on **starports that already have hightech** from another link (Snail); ornamental starports with hightech 0 (Gold/dec_truss) show relay in UI only |
| Security install economy apply | Security weak **Military** +5% on all receivers (Update 3 / Mega Guide: weak links to all non-local ports) |
| Link UI weak counts | `MarketLinks` + `calcSiteEconomies` show applied weak (audit) for agriculture; cap non-agriculture display at 1 per economy |
| Body link primary | Dockable ports win over hubs/installations; hubs only primary when the body has no port; installations never primary |
| Hub grandchild strong | Hub `strongSites` recurse with `subLink: '*'` so settlements keep their own economy (extraction/industrial), not the hub’s |
| Farm → port strong | Same-body demeter/picumnus subordinate to the port; strong-link at T1 coefficient (0.4), not full facility economy |
| Cluster farm strong | Sibling-moon demeter/picumnus skip outward strong when a colony port has `parentLink` to the farm |
| Reserve boost on strong | `applyStrongLinkBoost` MAJOR/PRISTINE (and extraction volcanism) **per strong-link contribution**; hightech/tourism body boosts still once per port calc |
